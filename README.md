...
1. In 1-2 sentences, explain what React's `useRouteMatch` hook is used for.
    ...
    -useRouteMatch is kind of a shortcut so that we dont have to type every single aspect of the url every single time. instead, we only need to type out the parts of the url that are changing in the instance which we are referring to. most of the time, everything else is staying the same.

2. How would you explain form validation to someone who has never programmed before?

    - form validation is really just making sure that the correct responses are entered into the spaces provided on the spaces in the form. they make sure that the user knows what is expected in those spaces and the computer knows what and what not to accept as a response.

3. In 1-2 sentences, define end to end testing.

    - end to end testing basically runs the program like a user would and ensures that everything is running smoothly to ensure that there are no bugs or glitches that need to be reported by the user. it goes through the channels that the computer normally would to accomplish those things and carries out the commands assigned to see if they can be accomplished with the code provided.
...
- [x] A homepage that has a "/" route and links to your form (button, nav bar, or any other type of link is acceptable but must have an id of "order-pizza") **(App.js is my homepage!)**
